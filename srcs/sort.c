...
void	sort(t_stack **stack_a)
{
	...
	else
	{
		...
		if (i < last_element_size(*stack_a) - i)
		{
...
/*
 ...
 sort_b pushes one by one all the elements in stack_a to the stack_b, until only three elements are left in stack_a. 
 While pushing, it makes sure if the stack_b is sorted. 
 When three elements are left, it calls the sort_till_three function to sort left over elements in stack_a.

 sort_till_three sorts and pushes stacks until 3 members left behind.

 sort_a is pushing back the elements from stack_b to stack_a until stack_b is empty. 
